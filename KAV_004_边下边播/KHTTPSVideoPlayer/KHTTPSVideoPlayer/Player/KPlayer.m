...
#import "KRequest.h"
#import "KHudManager.h"

#define kScreenHeight ([UIScreen mainScreen].bounds.size.height)
#define kScreenWidth ([UIScreen mainScreen].bounds.size.width)
#define IOS_VERSION  ([[[UIDevice currentDevice] systemVersion] floatValue])
NSString *const kPlayerStateChangedNotification    = @"kPlayerStateChangedNotification";
NSString *const kPlayerProgressChangedNotification = @"kPlayerProgressChangedNotification";
NSString *const kPlayerLoadProgressChangedNotification = @"kPlayerLoadProgressChangedNotification";

@interface KPlayer ()<KResLoadingDelegate>

@property (nonatomic        ) KPlayerState  state;
...
@property (nonatomic, strong) KRequest *request;
...
- (void)playWithUrl:(NSURL *)url showView:(UIView *)showView
{
    ...
    if (IOS_VERSION < 7.0 || ![str hasPrefix:@"http"]) {
        ...
        self.request          = [[KRequest alloc] init];
        self.request.delegate = self;
        NSURL *playUrl              = [_request getSchemeVideoURL:url];
        self.videoURLAsset             = [AVURLAsset URLAssetWithURL:playUrl options:nil];
        [_videoURLAsset.resourceLoader setDelegate:_request queue:dispatch_get_main_queue()];
    ...
    if ([url.scheme isEqualToString:@"file"]) {
        ...
        if (self.state == KPlayerStatePlaying) {
            [[NSNotificationCenter defaultCenter] postNotificationName:kPlayerStateChangedNotification object:nil];
        } else {
        ...
        if (self.state == KPlayerStateBuffering) {
            [[NSNotificationCenter defaultCenter] postNotificationName:kPlayerStateChangedNotification object:nil];
        } else {
    ...
    [[KHudManager sharedInstance] showHudOnView:_showView caption:nil image:nil acitivity:YES autoHideTime:0];
...
{
    if (self.state == KPlayerStateStopped) {
        return;
    }
...
- (void)resumeOrPause
{
    ...
    if (self.state == KPlayerStatePlaying) {
        ...
        [self.player pause];
        self.state = KPlayerStatePause;
    } else if (self.state == KPlayerStatePause) {
...
- (void)monitoringPlayback:(AVPlayerItem *)playerItem
{
    ...
    self.playbackTimeObserver = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1) queue:NULL usingBlock:^(CMTime time) {
        ...
        if (strongSelf.isPauseByUser == NO) {
            strongSelf.state = KPlayerStatePlaying;
        }
...
{
    if (state != KPlayerStateBuffering) {
        [[KHudManager sharedInstance] hideHud];
    }
